...
$config[:canvas] = YAML.load File.open("config/canvas.yml")
...
SCHEDULER.every '3h', :first_in => '45s' do
  enrollments = get_enrollments($config[:canvas][:url_base] + $config[:canvas][:enrollments][:path], $config[:canvas][:auth_token])
  data = []
